...
def sync_target(agents: list[dict], out_dir: Path, suffix: str, emit, label: str,
    ...
    def warn(msg: str) -> None:  # always shown, even in --quiet
    ...
    for stale in sorted(prev_set - set(wanted)):
        p = out_dir / stale
        if p.exists():
            say(f"prune {stale}")
            if not dry_run:
    ...
    for fname, content in sorted(wanted.items()):
        dest = out_dir / fname
        # Never clobber a file we didn't generate: an existing target that isn't
        # in our prior manifest is hand-authored (or from another tool) and just
        # happens to share a name with a canonical subagent. Skip + warn, and
        # keep it out of the manifest so a later prune can't delete it either.
        if dest.exists() and fname not in prev_set:
            warn(f"{out_dir}/{fname} exists and was not generated by agent-scripts "
                 f"— not overwriting (rename the canonical subagent to resolve the collision)")
            continue
