...
- For screenshot/GIF/reel artifacts, prefer the `ce-demo-reel` upload pattern when available: temporary Litterbox preview is fine for human review, but final PR proof must use permanent public hosting such as configured R2 or Catbox (`files.catbox.moe`) and must be embedded directly in the PR body with Markdown image/video syntax, not listed as a bare URL. Verify the GitHub PR renders the artifact inline before calling proof complete.
