...
4. Verify locally and live end-to-end when possible. For macOS UI behavior, use the `peekaboo` skill for screenshots / UI proof; for web UI, use the `agent-browser` skill. When visual proof applies, make the artifact render inline in the PR body or proof comment: use durable public hosting plus Markdown image syntax for screenshots/GIFs, or GitHub-hosted uploaded attachments for video reels when GitHub renders them inline. A bare Catbox/R2/GitHub attachment URL is not enough. For API/provider behavior, use a real usable key/account through the expected secret workflow when available. If access is missing, stop before pretending the item is done and ask Ossian for help.
...
7. After every landed PR, post a PR comment with exactly how it was tested: local commands, live/UI/API proof, CI run/check state, landed commit, and any caveats. If verification images apply, attach or embed them directly in the comment; if durable inline rendering is unavailable, say so and include the exact proof blocker instead of silently omitting them or dropping a bare local path.
