---
name: plan
description: Produce a durable implementation plan — optionally with a full product spec (user stories + contracts) — and save it to docs/plans/. Defines HOW to build something, and WHAT it must do. Use when the user wants a plan, an implementation breakdown, a PRD/spec, user stories, to scope a feature/refactor/migration before coding, or says "plan this", "write a plan", "write a PRD", "spec this out", "break this down". Runs standalone; pairs with brainstorm / grill-me upstream when the idea needs sharpening.
---
